...
AUTHOR   : \x1b[1;93mBILAL BALOCH 🌝 \x1b[1;92mIBALOCH 
...
def work():
    ...
    try:
        ...
        if '"status":"Success"' in gzip:
            success += 1
            accounts.append(username + f':{G}@Bilal0861601 (BILAL){R}')
            with open('SafeumAccounts.txt', 'a') as f: 
                f.write(username + f":{G}@Bilal0861601 (BILAL){R}\n")
        else:
            failed += 1
    except:
        retry += 1
